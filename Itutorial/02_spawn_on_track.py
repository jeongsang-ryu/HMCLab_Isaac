"""Tutorial 02 — spawn one vehicle on the mini banked oval (duct map).

Supports `--robot f1tenth|mushr`. Track: mini_oval (banked + elevated).
World = GroundPlane + duct_track. Two captures per run:
  - top-down over the full mini oval
  - rear-top chase cam following the vehicle

Run:
    OMNI_KIT_ACCEPT_EULA=YES python Itutorial/02_spawn_on_track.py --robot f1tenth
"""
...
from _common import DEFAULT_TRACK, get_robot_spec  # noqa: E402
...
def main() -> int:
    ...
    tag_topdown = f"tut02_track_{spec.name}_topdown"
    tag_chase = f"tut02_track_{spec.name}_chase"
    ...
    spawn_duct_track(
        track,
        prim_path="/World/DuctTrack",
        pipe_radius=0.15,
        pipe_offset=1.1,
        rib_spacing=0.8,
    )
    print(f">>> track {track.name}: {track.num_points} pts, length={track.length():.2f} m", flush=True)
    ...
    # Camera centres & sizes from the track bounds (mini_oval is ~20×10)
    ...
    chase = ChaseCamRecorder(
        ...
        behind=2.5, above=1.5, look_up=0.2,
    )
    ...
    print(
        ...
        f"speed={ego.data.root_lin_vel_w[0].norm().item():.2f}",
        flush=True,
    )
    print(f"TUT02_OK robot={spec.name} track={track_name}", flush=True)
    topdown.finalize()
    chase.finalize()
    return 0
